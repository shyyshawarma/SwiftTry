...
class TryOnController:
    ...
    def tryon_image(
        ...
        save_dir = f"./output_result/"
        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)
        date_str = datetime.now().strftime("%Y%m%d")
        time_str = datetime.now().strftime("%H%M")
        out_path = os.path.join(save_dir, f"{date_str}T{time_str}.png")
    ...
    def tryon_image_batch(
        ...
        for idx in range(len(images)):
            image = images[idx]
            if repaint:
                repaint_img = np.array(Image.open(image_paths[idx]).resize((width, height))) # [0, 255]
                repaint_mask_img = np.array(masks[idx])[..., None]
                repaint_mask_img = repaint_mask_img.astype(np.float32) / 255.0
                repaint_mask_img[repaint_mask_img < 0.5] = 0
                repaint_mask_img[repaint_mask_img >= 0.5] = 1
                image = images[idx] * repaint_mask_img + repaint_img * (1-repaint_mask_img)
            res_image_pils.append(Image.fromarray(image.astype(np.uint8)))
...
def batchify_pairs(pairs, batch_size=16):
    batches = [pairs[i:i + batch_size] for i in range(0, len(pairs) - 1, batch_size)]
    return batches
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data_dir", type=str, default="/root/dataset/zalando-hd-resized/test")
    parser.add_argument("--test_pair_txt", type=str, default="/root/dataset/zalando-hd-resized/test_pairs.txt")
    parser.add_argument("--batch_size", type=int, default=1)
    args = parser.parse_args()
    save_dir = f"./output_result/test_vitonhd_1024x768_ft_upblocks_unpaired_200k_aug"
    print(save_dir)
    controller = TryOnController(args.config)
    
    with open(args.test_pair_txt, 'r') as f:
        test_pairs = f.read()

    test_pairs = test_pairs.split('\n')
    for batch_pair in batchify_pairs(test_pairs, args.batch_size):
        batch_pair = [pair.split() for pair in batch_pair if pair]
        ref_cloth_images = [join(args.data_dir, "cloth", pair[1]) for pair in batch_pair]
        image_paths = [join(args.data_dir, "image", pair[0]) for pair in batch_pair]
        masked_image_paths = [join(args.data_dir, "agnostic-v3.2", pair[0]) for pair in batch_pair]
        mask_paths = [join(args.data_dir, "agnostic-mask", pair[0].replace('.jpg', '_mask.png')) for pair in batch_pair]
        pose_paths = [join(args.data_dir, "image-dwpose", pair[0]) for pair in batch_pair]
        if os.path.exists(join(save_dir, 'canvas', f'{batch_pair[0][0]}_{batch_pair[0][1]}.png')):
            print(f"Skipping {batch_pair[0][0]} due to processed...")
            continue
        controller.tryon_image_batch(
            ref_cloth_images,
            image_paths,
            masked_image_paths,
            mask_paths,
