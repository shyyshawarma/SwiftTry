...
from src.data.dataset import VITONHDDataset, TikTokDressSingleDataset
...
def main(cfg):
    ...
    if config.solver.mixed_precision == "fp16":
        # only upcast trainable parameters (LoRA) into fp32
        cast_training_params(denoising_unet, dtype=torch.float32)

    lora_layers = filter(lambda p: p.requires_grad, denoising_unet.parameters())
    ...
    train_dataset = TikTokDressSingleDataset(
        data_root_dir=cfg.data.meta_paths,
        img_H=cfg.data.train_height,
        img_W=cfg.data.train_width
    )
    train_dataset_additional = TikTokDressSingleDataset(
        data_root_dir=cfg.data.meta_paths_2,
        img_W=cfg.data.train_width,
        img_H=cfg.data.train_height
    )
    train_dataset = torch.utils.data.ConcatDataset([train_dataset, train_dataset_additional])
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % cfg.checkpointing_steps == 0 or global_step == 1000:
                    if accelerator.is_main_process:
                        ...
                        unwrapped_unet = accelerator.unwrap_model(denoising_unet)
                        unet_lora_state_dict = convert_state_dict_to_diffusers(
                            get_peft_model_state_dict(unwrapped_unet)
                        )
                        accelerator.save(unet_lora_state_dict, os.path.join(save_path, f"lora_unet.pth"))
