...
from src.models_window_avg.pose_guider import PoseGuider
from src.models_window_avg.unet_2d_condition import UNet2DConditionModel
from src.models_window_avg.unet_3d import UNet3DConditionModel
from src.pipelines.pipeline_tryon_video_random_window_avg import TryOnVideoPipeline
...
class TryOnController:
    ...
    def tryon_video(
        ...
        # save_videos_grid(overlapped_video, out_path + 'overlapped.mp4', n_rows=1, fps=8)
        # save_videos_grid(nonoverlapped_video, out_path + 'nonoverlapped.mp4', n_rows=1, fps=8)
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="./configs/prompts/tryon_video_window_avg.yaml")
    parser.add_argument("--data_dir", type=str, default="/root/dataset/VVT_video")
    parser.add_argument("--test_pairs", type=str, default="/root/dataset/VVT_video/test_pairs_test_paired.txt")
    args = parser.parse_args()
    controller = TryOnController(args.config)
    save_dir = "./output_result/test_vvt_vvt_512_paired(oot)_window_attn_w=3_avg"
    # read txt file
