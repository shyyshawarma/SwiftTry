...
from src.models_window_w5.mutual_self_attention import ReferenceAttentionControl
from src.models_window_w5.pose_guider import PoseGuider
from src.models_window_w5.unet_2d_condition import UNet2DConditionModel
from src.models_window_w5.unet_3d import UNet3DConditionModel
from src.pipelines.pipeline_tryon_video_random_window_w5 import TryOnVideoPipeline
from src.utils.calculate_ssim_lpips import compute_ssim_l1_psnr, compute_lpips
from src.utils.utils_flatten import create_indices_trajectories
...
def add_caption_to_video(np_videos, lpips_score, ssim_score):
    if lpips_score is None and ssim_score is None:
        caption = ""
    else:
...
def main(cfg):
    ...
    # create indices trajectories
    start = time.time()
    trajs = create_indices_trajectories(cfg.data.train_height, cfg.data.train_width, cfg.data.n_sample_frames, device='cuda')
    end = time.time()
                        ...
                        for tracker in accelerator.trackers:
                            if tracker.name == "tensorboard":
                                ...
                                for d in sample_dicts:
                                    ...
                                    name = d['name']
                                    np_videos_with_caption = add_caption_to_video(np_videos.cpu().numpy(), d.get('lpips'), d.get('ssim'))
