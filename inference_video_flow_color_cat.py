...
class TryOnController:
    ...
    def tryon_video(
        ...
        for batch in batches:
            ...
            predicted_flows.append(flow_extractor.extract_optical_flows([image_list[b] for b in batch], 'cuda'))
        predicted_flows = torch.cat(predicted_flows, dim=0)
        predicted_flows = flow_to_image(predicted_flows)
        predicted_flows = predicted_flows / 127.5 - 1
        predicted_flows = rearrange(predicted_flows.unsqueeze(0), "b f c h w -> b c f h w").to(self.weight_dtype)
        video = self.pipeline(
            ref_cloth_image,
...
if __name__ == '__main__':
    ...
    parser.add_argument("--config", type=str, default="./configs/prompts/tryon_video_flow_cat.yaml")
    parser.add_argument("--data_dir", type=str, default="/root/dataset/VVT_video")
    parser.add_argument("--test_pairs", type=str, default="/root/dataset/VVT_video/test_pairs_test.txt")
    args = parser.parse_args()
    controller = TryOnController(args.config)
    save_dir = "./output_result/test_tiktokdress_flow_color_cat_vvt_512_unpaired(oot)"
    # read txt file
