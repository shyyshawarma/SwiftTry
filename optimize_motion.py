...
from src.models.motion_embedding import inject_motion_embeddings, save_motion_embeddings, load_motion_embeddings
...
def log_validation(
    ...
    motion_embed = torch.load(motion_embedding_path)
    load_motion_embeddings(pipe.denoising_unet, motion_embed)
    ref_cloth_image_paths = sorted(glob.glob('./examples/clothes/*'))
    pose_video_paths = ['./examples/videos/00333/00333_resized_kps.mp4']
    masked_video_paths = ['./examples/videos/00333/00333_resized_masked.mp4']
    mask_video_paths = ['./examples/videos/00333/00333_resized_mask.mp4']
    video_paths = ['./examples/videos/00333/00333_resized.mp4']
    results = []
...
def main(cfg):
    ...
    logger.info(f"Total trainable params {len(trainable_params)} before injecting motion embedding")
    inject_motion_embeddings(net.denoising_unet)
    trainable_params = list(filter(lambda p: p.requires_grad, net.parameters()))
    logger.info(f"Total trainable params {len(trainable_params)} after injecting motion embedding")
    # vae.enable_slicing()
    ...
    train_dataset = SingleVideoDataset(
        ...
        single_video_path=cfg.data.single_video_path,
        single_video_masked_path=cfg.data.single_video_masked_path,
        single_video_mask_path=cfg.data.single_video_mask_path,
        single_video_dwpose_path=cfg.data.single_video_dwpose_path,
        single_garment_image=cfg.data.single_garment_image
    )
    ...
    loss_func = DebiasedTemporalLoss()
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                if cfg.snr_gamma == 0:
                    loss = loss_func.forward(model_pred.float(), target.float())
                else:
                ...
                train_loss_temporal += avg_loss.item() / cfg.solver.gradient_accumulation_steps
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % cfg.checkpointing_steps == 0 or global_step == 20:
                    if accelerator.is_main_process:
                        ...
                        save_motion_embeddings(unwrap_net.denoising_unet, os.path.join(save_path, 'motion_embed.pt'))
                        ...
                        sample_dicts = log_validation(
                            ...
                            generator=generator,
                            motion_embedding_path=os.path.join(save_path, 'motion_embed.pt')
                        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/train/stage2_optimize.yaml")
