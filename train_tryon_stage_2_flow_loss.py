...
class BidirectionalFlowLoss(nn.Module):
    def __init__(self, device='cuda'):
        super(BidirectionalFlowLoss, self).__init__()
        self.flow_extractor = FlowExtractor(device=device)

    def compute_foward_flows(self, input_video):
        foward_flow = self.flow_extractor.extract_optical_flows_tensor(input_video, backward=False)
        backward_flow = self.flow_extractor.extract_optical_flows_tensor(input_video, backward=True)
        return foward_flow, backward_flow
        
    def forward(self, video_pred, video_gt, masks=None):
        forward_flow_pred, backward_flow_pred = self.compute_foward_flows(video_pred)
        forward_flow_gt, backward_flow_gt = self.compute_foward_flows(video_gt)
        
        if masks is None:
            masks = torch.ones_like(forward_flow_pred)
        loss = torch.sum(torch.abs(forward_flow_pred - forward_flow_gt), dim=0) + torch.sum(torch.abs(backward_flow_pred - backward_flow_gt), dim=0)
...
def main(cfg):
    ...
    flow_loss = BidirectionalFlowLoss('cuda')
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                with torch.no_grad():
                    ...
                    gt_video = rearrange(
                        pixel_values_vid, "b f c h w -> b c f h w"
                    )
                    # video
                ...
                # reconstruct original video
                orig_video_latent = pred_original_video(
                    train_noise_scheduler,
                    model_pred,
                    timesteps,
                    x_t
                )
                orig_video_pred = None
                with torch.no_grad():
                    orig_video_pred = decode_latents(vae, orig_video_latent.to(weight_dtype))
                
                if cfg.snr_gamma == 0:
                    loss = F.mse_loss(
                        model_pred.float(), target.float(), reduction="mean"
                    ) + 0.1 * flow_loss(orig_video_pred.float(), gt_video.float(), pixel_values_mask_vid.float())
                else:
                    ...
                    loss = loss.mean() + 0.1 * flow_loss(orig_video_pred.float(), gt_video.float(), pixel_values_mask_vid.float())
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/train/stage2_vvt_flow_loss.yaml")
