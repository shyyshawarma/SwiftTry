...
from src.utils.util import read_frames, save_videos_grid, add_caption_to_video
...
class TryOnController:
    ...
    def tryon_video(
        ...
        video = add_caption_to_video(video, "overlapped!", overlapped_frame_ids)
        video_full = add_caption_to_video(video_full, "overlapped!", overlapped_frame_ids)
        if not os.path.exists(save_dir):
            os.makedirs(save_dir, exist_ok=True)
        video_full_out_dir = os.path.join(save_dir, 'canvas')
        out_dir = os.path.join(save_dir, 'result')
        video_full_out_path = os.path.join(video_full_out_dir, f"{video_name}-{cloth_name}_C4_avg.mp4")
        out_path = os.path.join(out_dir, f"{video_name}-{cloth_name}_C4_avg.mp4")
...
if __name__ == '__main__':
    ...
    controller.tryon_video(
        ...
        pose_video_path=args.video_path[:-4] + '_kps.mp4',
        clip_length=100000,
        repaint=False,
        save_dir=save_dir
    )
