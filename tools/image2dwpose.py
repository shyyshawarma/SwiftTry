import concurrent.futures
import os
import random
from pathlib import Path
import sys
from PIL import Image
sys.path.append('/root/Projects/Moore-AnimateAnyone')
import numpy as np
from tqdm import tqdm

from src.dwpose import DWposeDetector
from src.utils.util import get_fps, read_frames, save_videos_from_pil

# Extract dwpose mp4 videos from raw videos
# /path/to/video_dataset/*/*.mp4 -> /path/to/video_dataset_dwpose/*/*.mp4


def process_single_image(image_path, detector, root_dir, save_dir):
    relative_path = os.path.relpath(image_path, root_dir)
    # print(relative_path, image_path, root_dir)
    out_path = os.path.join(save_dir, relative_path)
    if os.path.exists(out_path):
        print(f'{relative_path} exists, skipping...')
        try:
            Image.open(out_path)
            return
        except:
            pass

    output_dir = Path(os.path.dirname(os.path.join(save_dir, relative_path)))
    if not output_dir.exists():
        output_dir.mkdir(parents=True, exist_ok=True)

    
    frames = [Image.open(image_path)]
    kps_results = []
    for i, frame_pil in enumerate(frames):
        size = frame_pil.size
        result, score = detector(frame_pil)
        score = np.mean(score, axis=-1)
        result = result.resize(size)
        kps_results.append(result)
    for res in kps_results:
        res.save(out_path)
    #save_videos_from_pil(kps_results, out_path, fps=fps)


def process_batch_images(image_list, detector, root_dir, save_dir):
    for i, image_path in tqdm(enumerate(image_list)):
        print(f"Process {i}/{len(image_list)} video")
        process_single_image(image_path, detector, root_dir, save_dir)


if __name__ == "__main__":
    # -----
    # NOTE:
    # python tools/extract_dwpose_from_vid.py --video_root /path/to/video_dir
    # -----
    import argparse

    parser = argparse.ArgumentParser()
    parser.add_argument("--image_dir", type=str)
    parser.add_argument(
        "--save_dir", type=str, help="Path to save extracted pose videos"
    )
    parser.add_argument("-j", type=int, default=48, help="Num workers")
    args = parser.parse_args()
    num_workers = args.j
    if args.save_dir is None:
        save_dir = args.image_dir + "_dwpose"
    else:
        save_dir = args.save_dir
    if not os.path.exists(save_dir):
        os.makedirs(save_dir)
    cuda_visible_devices = os.environ.get("CUDA_VISIBLE_DEVICES", "0,1")
    gpu_ids = [int(id) for id in range(len(cuda_visible_devices.split(",")))]
    print(f"avaliable gpu ids: {gpu_ids}")

    # collect all video_folder paths
    image_paths = set()
    for root, dirs, files in os.walk(args.image_dir):
        for name in files:
            if name.endswith(".png") or name.endswith(".jpg"):
                image_paths.add(os.path.join(root, name))
    image_paths = list(image_paths)
    random.shuffle(image_paths)
    
    # split into chunks,
    batch_size = (len(image_paths) + num_workers - 1) // num_workers
    print(f"Num images: {len(image_paths)} {batch_size = }")
    images_chunks = [
        image_paths[i : i + batch_size]
        for i in range(0, len(image_paths), batch_size)
    ]

    with concurrent.futures.ThreadPoolExecutor() as executor:
        futures = []
        for i, chunk in enumerate(images_chunks):
            # init detector
            gpu_id = gpu_ids[i % len(gpu_ids)]
            detector = DWposeDetector()
            # torch.cuda.set_device(gpu_id)
            detector = detector.to(f"cuda:{gpu_id}")

            futures.append(
                executor.submit(
                    process_batch_images, chunk, detector, args.image_dir, save_dir
                )
            )
        for future in concurrent.futures.as_completed(futures):
            future.result()
