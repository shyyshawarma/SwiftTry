...
if __name__ == '__main__':
    ...
    save_dir = f"./output_result/test_tiktok_new_SAM2mask_unpaired_averaging_C4"
    # read txt file
    ...
    for video_id, cloth_id in tqdm(test_pairs):
        ...
        if os.path.exists(os.path.join(save_dir, 'canvas', f"{video_id}-{cloth_name}.mp4")):
            print(f"{video_id}-{cloth_id}.png.mp4 already processed...")
            continue
        controller.tryon_video(
            ref_cloth_image=cloth_path,
            video_path=join(args.data_dir, "videos_all_normalized", f"{video_id}"),
            masked_video_path=join(args.data_dir, "videos_all_normalized_masked_new", f"{video_id}"),
            mask_video_path=join(args.data_dir, "videos_all_normalized_mask_new", f"{video_id}"),
            pose_video_path=join(args.data_dir, "videos_all_normalized_dwpose", f"{video_id}"),
            clip_length=10000,
            repaint=True,
            save_dir=save_dir,
        )
