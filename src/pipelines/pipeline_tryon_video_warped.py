...
from src.utils.utils_flatten import sample_trajectories
from src.models.mutual_self_attention import ReferenceAttentionControl
from src.pipelines.context import get_context_scheduler


def warp_latents(first_latent, optical_flow):
    """
    Warp subsequent latent frames using optical flow.

    Args:
    - first_latent (torch.Tensor): First latent frame tensor with shape (b, 4, 1, h, w).
    - optical_flow (torch.Tensor): Optical flow tensor with shape (b, N-1, h, w, 2).
    
    Returns:
    - warped_latents (torch.Tensor): Warped latent frames with shape (b, 4, N, h, w).
    """
    optical_flow = optical_flow.to(dtype=first_latent.dtype)
    b, _, _, h, w = first_latent.size()
    N = optical_flow.size(1) + 1
    
    # Initialize list to store warped latent frames
    warped_latents = [first_latent]

    # Generate grid of coordinates
    grid_coords = torch.meshgrid(
        torch.arange(h, device=first_latent.device),
        torch.arange(w, device=first_latent.device)
    )
    grid_coords = torch.stack(grid_coords, dim=-1).unsqueeze(0).float()  # (1, h, w, 2)
    
    # Repeat grid for each batch and each frame
    grid_coords = grid_coords.unsqueeze(0).repeat(b, 1, 1, 1, 1)  # (b, 1, h, w, 2)
    grid_coords = grid_coords.to(dtype=first_latent.dtype)
    # Iterate through optical flow frames and warp latents
    for i in range(N - 1):
        # Compute new grid using optical flow
        new_coords = grid_coords + optical_flow[:, i]
        
        # Reshape and normalize grid coordinates
        new_coords = new_coords.view(b, h, w, 2)
        new_coords = 2 * new_coords / torch.tensor([h - 1, w - 1], device=first_latent.device) - 1
        # Perform grid sampling to warp latent frame
        warped_latent = F.grid_sample(first_latent.squeeze(2), new_coords, align_corners=True)
        # Append warped latent frame to the list
        warped_latents.append(warped_latent.unsqueeze(2))

    # Concatenate along the N dimension
    warped_latents = torch.cat(warped_latents, dim=2)
...
class TryOnVideoPipeline(DiffusionPipeline):
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            if warping_flow is not None:
                warping_flow = warping_flow.permute(0, 2, 3, 1) # n, 2, h, w -> n, h, w, 2
                warping_flow = warping_flow.unsqueeze(0)
                latents = warp_latents(latents, warping_flow)
                # print(latents.shape)
            else:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        trajs = sample_trajectories(
            pil_list=images,
            device='cuda', return_flow_only=True
        )
        for k in trajs.keys():
            trajs[k] = trajs[k].to('cuda')
        predicted_flow_resolu = trajs['predicted_flow(32, 24)']
        print(trajs.keys())
        ...
        latents, image_latents = self.prepare_latents(
            ...
            device,
            generator,
            warping_flow=predicted_flow_resolu
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                #         ]
                #     )
                for ctx_idx, context in enumerate(global_context):                    
        ...
        return TryOnVideoPipelineOutput(videos=video)
