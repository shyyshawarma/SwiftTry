...
class TryOnVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        flow_fea = self.flow_guider(predicted_flows)
        flow_fea = (torch.cat([flow_fea] * 2) if do_classifier_free_guidance else flow_fea).to(torch.float16)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for context in global_context:
                    ...
                    flow_fea_input = torch.cat([flow_fea[:, :, c] for c in context])
                ...
                noise_rectification_period = (0.2, 0.8)
                noise_rectification_weight_start_omega = 0.1
                noise_rectification_weight_end_omega = 0.9
