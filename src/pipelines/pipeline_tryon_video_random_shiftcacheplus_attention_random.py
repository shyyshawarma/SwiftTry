...
def tensor_to_rgb_gif(feature_maps, gif_path="feature_map.gif", output_size=(512, 384)):
    feature_map = rearrange(feature_maps, "b c f h w -> b f h w c")[1]
    ...
    resize_transform = T.Resize(output_size, antialias=True)

    for i in range(feature_map.shape[0]):  
        frame = feature_map_np[i]
        frame_reshaped = frame.reshape(-1, frame.shape[-1]) 
        # Apply PCA to reduce to 3 channels
        pca = PCA(n_components=3)
        frame_pca = pca.fit_transform(frame_reshaped)
        
        frame_rgb = frame_pca.reshape(h, w, 3)
        frame_rgb = (frame_rgb - frame_rgb.min()) / (frame_rgb.max() - frame_rgb.min())
        frame_rgb_tensor = torch.tensor(frame_rgb).permute(2, 0, 1).unsqueeze(0)

        frame_resized = resize_transform(frame_rgb_tensor).squeeze(0).permute(1, 2, 0).numpy() 

        frames.append((frame_resized * 255).astype(np.uint8))
                ...
                for context_idx, context in enumerate(global_context):
                    ...
                    if i == 0 and i == len(timesteps) - 1: # full compute
                        ...
                        if drop != 0: # partial compute
                            ...
                            # os.makedirs(f"feature_maps0.8/{str(t.item())}", exist_ok=True)
                            # tensor_to_rgb_gif(cache_feat, f"feature_maps0.8/{str(t.item())}/{str(context)}.png")
                            # approximate the blended cached features
                            if drop == 1:
                                cache_feat = blend_with_keyframe(prev_cache_features, cache_feat, initial_weight=0.5)
                            # tensor_to_rgb_gif(cache_feat, f"feature_maps0.8/{str(t.item())}/{str(context)}-after.png")
                            cache_timestep = (
