...
class TryOnVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # initialize latents_timestep cache
        latents_timestep = torch.stack([timesteps[0]] * latents.shape[2]).to(device=device)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                pattern = [step * stride for step in range(context_frames//stride + 1)]
                ...
                non_zero_noise_pred_indicator = torch.zeros((latents.shape[2], ))

                for context_idx, context in enumerate(global_context):
                    if context_idx % drop_stride != 0:
                        # if not divided by drop_stride, reuse from cached latents
                        continue
                    ...
                    latents_timestep_input = (
                        torch.cat([latents_timestep[c] for c in context])
                        .to(device)
                    )
                    latents_timestep_input = latents_timestep_input.expand(batch_size, -1)
                    ...
                    for j, c in enumerate(context):
                        ...
                        non_zero_noise_pred_indicator[c] = True
                ...
                non_zero_noise_pred_indicator = non_zero_noise_pred_indicator.to(torch.bool)
                # compute the previous noisy sample x_t -> x_t-1
                latents[:, :, non_zero_noise_pred_indicator] = self.scheduler.step(
                    noise_pred[:, :, non_zero_noise_pred_indicator], latents_timestep[non_zero_noise_pred_indicator], t, latents[:, :, non_zero_noise_pred_indicator], **extra_step_kwargs, return_dict=False
                )[0]

                # update caching
                latents_timestep[non_zero_noise_pred_indicator] = timesteps[i + 1] if i < len(timesteps) - 1 else timesteps[i]
