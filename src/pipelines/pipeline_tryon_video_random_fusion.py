...
class TryOnVideoPipeline(DiffusionPipeline):
    ...
    def prepare_latents(
        ...
        if latents is None:
            ...
            latents = latents.repeat(1, 1, video_length // tile_size + 1, 1, 1)[:, :, :video_length]
        else:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        indices = [[*range(i, min(i + tile_size, video_length))] for i in
                   range(0, video_length - tile_size + 1, tile_size - tile_overlap)]
        if indices[-1][-1] < video_length - 1:
            indices.append([*range(video_length - tile_size, video_length)])
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                noise_pred = noise_pred.repeat(2 if do_classifier_free_guidance else 1, 1, 1, 1, 1)
                noise_pred_cnt = image_latents.new_zeros((video_length,))
                ...
                latent_pose_input = pose_fea

                for idx in indices:
                    _noise_pred = self.denoising_unet(
                        latent_model_input[:, :, idx],
                        t,
                        encoder_hidden_states=encoder_hidden_states,
                        pose_cond_fea=latent_pose_input[:, :, idx],
                        return_dict=False,
                        sampling_step=i,
                    )[0]
                    noise_pred[:, :, idx] += _noise_pred * weight[None, None, :, None, None]
                    noise_pred_cnt[idx] += weight
                    counter[:, :, idx] = counter[:, :, idx] + 1
                noise_pred.div_(noise_pred_cnt[None, None, :, None, None])
