...
import random
from src.pipelines.utils import interpolate_features, create_attention_mask, refine_noisy_latents
...
class TryOnVideoPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        prev_cache_timesteps = cache_timesteps.clone()
        prev_cache_latents = latents.clone()
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                # refine noisy latents
                latents = refine_noisy_latents(prev_cache_timesteps, cache_timesteps, prev_cache_latents, latents)
                ...
                for context_idx, context in enumerate(global_context):
                    if context_idx % 2 != 0 and i != 0 and i != len(timesteps) - 1:
                        ...
                        for j, c in enumerate(context):
                            prev_cache_timesteps[c] = cache_timesteps[c]
                            cache_timesteps[c] = t
                ...
                prev_cache_latents[:, :, cache_timesteps==t] = latents[:, :, cache_timesteps==t]
                
                # compute the previous noisy sample x_t -> x_t-1
                latents[:, :, cache_timesteps==t] = self.scheduler.step(
                    noise_pred[:, :, cache_timesteps==t], t, latents[:, :, cache_timesteps==t], **extra_step_kwargs, return_dict=False
                )[0]
