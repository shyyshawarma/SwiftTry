...
from .utils import save_caching_scheduler
...
import random
from src.pipelines.utils import  create_attention_mask
...
class TryOnVideoPipeline(DiffusionPipeline):
    ...
    def __init__(
        ...
        tomesd.apply_patch(self.denoising_unet, ratio=0.5)
    ...
    def __call__(
        ...
        **kwargs,
        ...
        # Init:
        #   - cache_features: [B, c, F, h, w], T timestep
        #   - video_latents v: [B, C, F, H, W], chunk_length l = 16
        #   - shifting scheduler S: array with T elements.
        #   - PC, FC (partial compute, full compute): PC_t(cache_features, latents{t+1}), FC_t(latents{t+1})
        #   
        #   for t in [T, 0]: # denoising
        #       for ci in [0, F // chunk_length]:
        #           latents_{ci, t+1} = video_latents[ci * l + S[t] : ci * l + S[t] + l] # assume F % l == 0
        #           if ci % 2 == 0 or t == T: # full compute
        #               cache_features_t, latents_{ci, t} = FC_t(latents_{ci, t+1})
        #               cache_features[ci * l + S[t] : ci * l + S[t] + l] = cache_features_t
        #           else: # partial compute
        #               cache_feat = cache_features[ci * l + S[t] : ci * l + S[t] + l]
        #               latents_{ci, t} = PC_t(cache_feat, latents_{ci, t+1})
        #           video_latents[ci * l + S[t] : ci * l + S[t] + l] = latents_{ci, t}
        #       
        #        video = VAE.decode(video_latents)
        cache_noise_pred = torch.zeros(
            (
            ...
            for i, t in enumerate(timesteps):
                ...
                for context_idx, context in enumerate(global_context):
                    ...
                    for j, c in enumerate(context):
                        ...
                        if not skip_unet: # caching epsilon in case of full&partial compute
                            cache_noise_pred[:, :, c] = noise_pred[:, :, c]
                        else:
                            noise_pred[:, :, c] = cache_noise_pred[:, :, c]
                        counter[:, :, c] = counter[:, :, c] + 1
                        cache_features[:, :, c] = cache_feat
                        skips[c] = 1 if skip_unet else 0
        ...
        save_caching_scheduler(caching_scheduler)
        # Post-processing
