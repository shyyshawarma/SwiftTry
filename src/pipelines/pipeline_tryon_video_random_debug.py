        ...
        **kwargs,
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if i == len(timesteps) - 1:
                    # counter which is overlapped
                    overlapped_mask = (counter == 2).squeeze()
                    nonoverlapped_mask = (counter != 2).squeeze()
                    overlapped_latents = latents[:, :, overlapped_mask, :, :]
                    nonoverlapped_latents = latents[:, :, nonoverlapped_mask, :, :]
        ...
        video_overlapped_chunks = self.decode_latents(overlapped_latents)
        video_nonoverlapped_chunks = self.decode_latents(nonoverlapped_latents)
        # Convert to tensor
        if output_type == "tensor":
            video = torch.from_numpy(video)
            video_overlapped_chunks = torch.from_numpy(video_overlapped_chunks)
            video_nonoverlapped_chunks = torch.from_numpy(video_nonoverlapped_chunks)

        if not return_dict:
            return video

        return TryOnVideoPipelineOutput(videos=video), video_overlapped_chunks, video_nonoverlapped_chunks
