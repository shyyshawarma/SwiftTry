...
class VersatileAttention(Attention):
    ...
    def forward(
        ...
        **kwargs,
        ...
        if segment_overlap == 0: # this means first segment
            self.k_cache = None
            self.v_cache = None

        # Calculate the start index for the current segment
        start_idx = segment_overlap

        # Extract the new segment to process
        new_segment = hidden_states[:, start_idx:]

        # Process the new segment
        new_key = self.to_k(new_segment)
        new_value = self.to_v(new_segment)

        # Concatenate with the cached K and V
        if self.k_cache is not None and self.v_cache is not None:
            key = torch.cat([self.k_cache[:, -start_idx:], new_key], dim=1)
            value = torch.cat([self.v_cache[:, -start_idx:], new_value], dim=1)
        else:
            key = new_key
            value = new_value

        # Update the caches with the current segment
        self.k_cache = key
        self.v_cache = value
