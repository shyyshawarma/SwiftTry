...
class UNet3DConditionModel(ModelMixin, ConfigMixin):
    ...
    def forward(
        ...
        **kwargs
        ...
        if skip_unet:
            if not return_dict:
                return (torch.zeros_like(sample[:, :4]), cache_features) # b, c, f, h, w
            return UNet3DConditionOutput(sample=torch.zeros_like(sample[:, :4]))
        ...
        # Branch: 4 down_blocks, each with 3 skip connections. 
        # Here we ignore the first skip branch, whose computations only has up_blocks but without down_blocks.
        ...
        if not return_dict:
            return (sample, cache_features) # b, c, f, h, w
