...
def preprocess(img1_batch, img2_batch, transforms, size=[512, 384]):
...
@torch.no_grad()
def sample_trajectories_only(predicted_flows, device):
    height, width = predicted_flows.shape[-2], predicted_flows.shape[-1]
    ...
    for resolution in resolutions:
        ...
        res['predicted_flow{}'.format(resolution)] = predicted_flow_resolu
    return res


def create_trajectories(H, W, T):
    """
    seqs = []
    for t in range(T):
        for x in range(H):
    ...
    return seqs
    """
    t = torch.arange(T).unsqueeze(1).unsqueeze(2).unsqueeze(3)
    x = torch.arange(H).unsqueeze(0).unsqueeze(2).unsqueeze(3)
    y = torch.arange(W).unsqueeze(0).unsqueeze(1).unsqueeze(3)
    
    # Broadcast these to create a grid of shape (T, H, W, 3)
    t_broadcasted = t.expand(T, H, W, 1)
    x_broadcasted = x.expand(T, H, W, 1)
    y_broadcasted = y.expand(T, H, W, 1)
    
    # Stack along the last dimension to create (T, H, W, 3)
    grid = torch.cat((t_broadcasted, x_broadcasted, y_broadcasted), dim=-1)
    
    # Repeat each element T times to get the final shape (T*H*W, T, 3)
    traj = grid.view(T * H * W, 1, 3).repeat(1, T, 1)
    return traj


@torch.no_grad()
def create_indices_trajectories(height, width, video_length, device='cuda'):
    resolutions = [(height // 8, width // 8), (height // 16, width // 16), (height // 32, width // 32), (height // 64, width // 64)]
    res = {}
    for resolution in resolutions:
        traj = create_trajectories(resolution[0], resolution[1], video_length)
        res["traj{}".format(resolution)] = traj.to(device)
        res["mask{}".format(resolution)] = torch.ones_like(res["traj{}".format(resolution)][..., 0]).to(torch.bool).to(device)
    return res
...
@torch.no_grad()
def sample_trajectories(pil_list, device, return_flow_only=False):
    ...
    current_frames, next_frames = preprocess(frames[clips[:-1]], frames[clips[1:]], transforms, size=[height, width])
