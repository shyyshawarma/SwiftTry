...
class VanillaTemporalModule(nn.Module):
    def __init__(
        ...
        inner_dim = num_attention_heads * in_channels

        self.conv_f = nn.Conv3d(in_channels*2, in_channels, kernel_size=(1, 1, 1))
        
        if zero_initialize:
            self.temporal_transformer.proj_out = zero_module(
                self.temporal_transformer.proj_out
            )
            self.conv_f = zero_module(
                self.conv_f
            )
    ...
    def forward(
        ...
        **kwargs
        ...
        if flows is not None and flows.shape == hidden_states.shape:
            hidden_states = self.conv_f(torch.cat([input_tensor, flows], dim=1))
