...
class LocalAttnProcessor:
    ...
    def __call__(
        ...
        **kwargs,
        ...
        key_unfolded = rearrange(key_unfolded, "(b f) (c k2) (h w) -> (b h w) f c k2", h=height, w=width, f=video_length, k2=self.window_size**2)
        value_unfolded = rearrange(value_unfolded, "(b f) (c k2) (h w) -> (b h w) f c k2", h=height, w=width, f=video_length, k2=self.window_size**2)

        # Compute average of key and value patches
        key = reduce(key_unfolded, 'bhw f c k2 -> bhw f c', 'mean')
        value = reduce(value_unfolded, 'bhw f c k2 -> bhw f c', 'mean')
