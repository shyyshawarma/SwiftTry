...
def save_images_folder(list_images, folder_path):
    if not os.path.exists(folder_path):
        os.makedirs(folder_path, exist_ok=True)
    for idx, image in enumerate(list_images):
        image.save(join(folder_path, "{:04d}".format(idx)))
...
if __name__ == '__main__':
    ...
    parser.add_argument("--cloth_name", type=str, default="00126_00")
    ...
    video_full_out_path = os.path.join(video_full_out_dir, f"{args.video_name}-{args.cloth_name}_s1")
    out_path = os.path.join(out_dir, f"{args.video_name}-{args.cloth_name}_s1")
    ...
    for image, masked, mask, pose in tqdm(zip(image_paths, masked_image_paths, mask_paths, pose_paths)):
        canvas, result = controller.tryon_image(ref_cloth_image, image, masked, mask, pose)
