...
class TryOnController:
    ...
    def tryon_video(
        ...
        image_list = [Image.open(image_pil).resize((width, height)) for image_pil in sorted(glob.glob(join(video_path, '*.png')))]
        masked_image_list = [Image.open(masked_image_pil).resize((width, height)) for masked_image_pil in sorted(glob.glob(join(masked_video_path, '*.png')))]
        mask_list = [Image.open(mask_pil).convert('L').resize((width, height)) for mask_pil in sorted(glob.glob(join(mask_video_path, '*.png')))]
        pose_list = [Image.open(pose_image_pil).resize((width, height)) for pose_image_pil in sorted(glob.glob(join(pose_video_path, '*.png')))]
...
if __name__ == '__main__':
    ...
    parser.add_argument("--test_pairs", type=str, default="/root/dataset/TikTokDress/test_unpaired.txt")
    args = parser.parse_args()
    controller = TryOnController(args.config)
    save_dir = "./output_result/test_tiktokdress_tiktokdress_flow_512_unpaired(oot)"
    # read txt file
    ...
    for video_id, cloth_id in tqdm(test_pairs):
        cloth_path = join(args.data_dir, "garments_normalized", cloth_id)
        cloth_name = os.path.basename(cloth_path)
        if os.path.exists(os.path.join(save_dir, 'canvas', f"{video_id}-{cloth_name}.mp4")):
            print(f"{video_id}-{cloth_id}.mp4 already processed...")
            continue
        controller.tryon_video(
            ref_cloth_image=cloth_path,
            video_path=join(args.data_dir, "images_all_normalized", video_id[:-4]),
            masked_video_path=join(args.data_dir, "images_all_normalized_masked", video_id[:-4]),
            mask_video_path=join(args.data_dir, "images_all_normalized_mask", video_id[:-4]),
            pose_video_path=join(args.data_dir, "images_all_normalized_dwpose", video_id[:-4]),
            clip_length=10000,
            repaint=False,
            save_dir=save_dir
        )
