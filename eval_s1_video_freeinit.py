...
from src.pipelines.freeinit_utils import get_freq_filter, freq_mix_3d
from diffusers.utils.torch_utils import randn_tensor


@torch.no_grad()
def init_filter(video_length, height, width, filter_params):
...
def freeinit_noise(batch_size, height, width, num_iters, latents, scheduler, freq_filter, video_length, device):
    print('Sampling with FreeInit...')
    ...
    for iter in range(num_iters):
        ...
        if iter == 0:
            ...
            current_diffuse_timestep = scheduler.config.num_train_timesteps - 1 # diffuse to t=999 noise level
            ...
            shape = (batch_size, 4, video_length, height // 8, width // 8)
            z_rand = randn_tensor(
                shape, device='cuda', dtype=torch.float32
            )
            # 3. Roise Reinitialization
            latents = freq_mix_3d(z_T.to(dtype=torch.float32), z_rand, LPF=freq_filter)
...
class TryOnController:
    def __init__(
        ...
        self.scheduler = None
        self.freeinit_latents = None
        
    
    def freeinit_noise(self):
        # freeinit
        video_length = 100
        height = 1024
        width = 768
        ...
        freq_filter = init_filter(video_length, height, width, filter_params)
        ...
        latents = freeinit_noise(batch_size=1, height=height, width=width, 
            num_iters=5, latents=latents, scheduler=self.scheduler, freq_filter=freq_filter, video_length=video_length, 
    ...
    def tryon_image(
        ...
        if self.pipeline is None:
            ...
            self.freeinit_latents = self.freeinit_noise()
        ...
        image = self.pipeline(
            ...
            guidance_scale=cfg,
            generator=generator,
            latents=self.freeinit_latents[:, :, frame_idx, ...]
        ).images
...
if __name__ == '__main__':
    ...
    parser.add_argument("--video_name", type=str, default="00339") 
    ...
    video_full_out_path = os.path.join(video_full_out_dir, f"{args.video_name}-{args.cloth_name}_s1_freeinit")
    out_path = os.path.join(out_dir, f"{args.video_name}-{args.cloth_name}_s1_freeinit")
    ...
    for image, masked, mask, pose in tqdm(zip(image_paths, masked_image_paths, mask_paths, pose_paths)):
        canvas, result = controller.tryon_image(ref_cloth_image, image, masked, mask, pose, frame_idx=no_frames)
    ...
    save_videos_from_pil(canvas_list, video_full_out_path)
    save_videos_from_pil(result_list, out_path)