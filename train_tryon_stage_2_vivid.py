...
from src.data.dataset import VVTDataset, TikTokDressDataset, ViViDDataset
...
def log_validation(
    ...
    ref_cloth_image_paths = sorted(glob.glob('/root/dataset/ViViD/ViViD_toy/upper_body/images/*'))
    pose_video_paths = sorted(glob.glob('/root/dataset/ViViD/ViViD_toy/upper_body/dwpose/*.mp4'))
    masked_video_paths = sorted(glob.glob('/root/dataset/ViViD/ViViD_toy/upper_body/agnostic/*.mp4'))
    mask_video_paths = sorted(glob.glob('/root/dataset/ViViD/ViViD_toy/upper_body/agnostic_mask/*.mp4'))
    video_paths = sorted(glob.glob('/root/dataset/ViViD/ViViD_toy/upper_body/videos/*.mp4'))
    results = []
    for test_case in zip(video_paths, masked_video_paths, mask_video_paths, pose_video_paths):
        ...
        for ref_cloth_path in ref_cloth_image_paths:
            ...
            if video_name.split('_')[0] != ref_name.split('_')[0]:
                lpips_score = 1.
                ssim_score = 0.
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/train/stage2_vivid.yaml")
