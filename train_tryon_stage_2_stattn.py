...
from src.data.dataset import VVTDataset, TikTokDressDataset, TikTokDressTripletDataset
from src.models_stattn.mutual_self_attention import ReferenceAttentionControl
from src.models_stattn.pose_guider import PoseGuider
from src.models_stattn.unet_2d_condition import UNet2DConditionModel
from src.models_stattn.unet_3d import UNet3DConditionModel
from src.pipelines.pipeline_tryon_video_random_stattn import TryOnVideoPipeline
...
class Net(nn.Module):
    def __init__(
        ...
        conditional_unet: UNet3DConditionModel,
        ...
        self.conditional_unet = conditional_unet
    ...
    def forward(
        ...
        # took out first and second frame
        first_latents = torch.cat([image_latents[:, :, 0:1, :, :], noisy_latents[:, 4:5, 0:1, :, :], noisy_latents[:, 5:, 0:1, :, :]], dim=1)
        prev_latents = torch.cat([image_latents[:, :, 1:2, :, :], noisy_latents[:, 4:5, 1:2, :, :], noisy_latents[:, 5:, 1:2, :, :]], dim=1)

        intermediate_features = None
        with torch.no_grad():
            first_features = self.conditional_unet(
                first_latents,
                torch.zeros_like(timesteps),
                pose_cond_fea=pose_fea,
                encoder_hidden_states=clip_image_embeds,
            ).intermediate_features
            prev_features = self.conditional_unet(
                prev_latents,
                torch.zeros_like(timesteps),
                pose_cond_fea=pose_fea,
                encoder_hidden_states=clip_image_embeds,
            ).intermediate_features
            intermediate_features = list(zip(first_features, prev_features))

        model_pred = self.denoising_unet(
            noisy_latents,
            timesteps,
            inter_features=intermediate_features,
            pose_cond_fea=pose_fea,
            encoder_hidden_states=clip_image_embeds,
        ).sample
...
def main(cfg):
    ...
    inference_config_path = "./configs/inference/inference_v2_stattn.yaml"
    ...
    conditional_unet = UNet3DConditionModel.from_pretrained_2d(
        cfg.base_model_path,
        "",
        subfolder="unet",
    ...
    conditional_unet.requires_grad_(False)
    ...
    train_dataset = TikTokDressTripletDataset(
        data_root_dir=cfg.data.meta_paths,
        img_W=cfg.data.train_width,
        img_H=cfg.data.train_height,
        max_window_range=cfg.data.max_window_range,
    )
    train_dataset_additional = TikTokDressTripletDataset(
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", type=str, default="./configs/train/stage2_tiktok_stattn.yaml")
