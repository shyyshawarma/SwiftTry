...
from tools.extract_optical_flow_from_vid import FlowExtractor
from src.data.dataset import VVTDataset, TikTokDressDataset
from src.models_cat.mutual_self_attention import ReferenceAttentionControl
from src.models_cat.pose_guider import PoseGuider
from src.models_cat.flow_guider import FlowEncoder
from src.models_cat.unet_2d_condition import UNet2DConditionModel
from src.models_cat.unet_3d import UNet3DConditionModel
from src.pipelines.pipeline_tryon_video_random_flow_cat import TryOnVideoPipeline
...
class Net(nn.Module):
    ...
    def forward(
        ...
        flow_cond_tensor = flows.to(device="cuda")
        pose_fea = self.pose_guider(pose_cond_tensor)
        flow_fea = self.flow_guider(flow_cond_tensor)
        if not uncond_fwd:
...
def log_validation(
    ...
    flow_guider = ori_net.flow_guider
    ...
    ref_cloth_image_paths = sorted(glob.glob('./configs/inference/videos/ref_cloth_images/*'))
    pose_video_paths = sorted(glob.glob('./configs/inference/videos/pose_videos/*.mp4'))
    masked_video_paths = sorted(glob.glob('./configs/inference/videos/masked_videos/*.mp4'))
    mask_video_paths = sorted(glob.glob('./configs/inference/videos/masks/*.mp4'))
    video_paths = sorted(glob.glob('./configs/inference/videos/videos/*.mp4'))
    results = []
    ...
    for test_case in zip(video_paths, masked_video_paths, mask_video_paths, pose_video_paths):
        ...
        for ref_cloth_path in ref_cloth_image_paths:
            ...
            start = random.randint(0, len(pose_images) - clip_length - 1)
            ...
            predicted_flows = flow_extractor.extract_optical_flows(image_list + [images[start+clip_length].resize((width, height))], device='cuda')
            predicted_flows = flow_to_image(predicted_flows)
            predicted_flows_vis = predicted_flows / 255.
            predicted_flows = predicted_flows / 127.5 - 1
            predicted_flows = rearrange(predicted_flows.unsqueeze(0), "b f c h w -> b c f h w")
            ...
            predicted_flows_vis = predicted_flows_vis.transpose(0, 1)
            predicted_flows_vis = predicted_flows_vis.unsqueeze(0)
            ...
            video = torch.cat([image_tensor, predicted_flows_vis.cpu(), cloth_tensor, video], dim=0) # (b, c, f, h, w)
            video = video.transpose(1, 2) # (b, f, c, h, w)
            video = rearrange(video, "b f c h w -> 1 f c h (b w)")
            results.append({"name": f"{ref_name}_{video_name}", "vid": video})
...
def main(cfg):
    ...
    flow_guider = FlowEncoder(guidance_embedding_channels=320, guidance_input_channels=3).to(device='cuda')
    ...
    # and flow guider too
    flow_guider.requires_grad_(True)
    ...
    train_dataset_additional = TikTokDressDataset(
        data_root_dir=cfg.data.meta_paths_2,
        img_W=cfg.data.train_width,
        img_H=cfg.data.train_height,
        sample_n_frames=cfg.data.n_sample_frames,
        sample_stride=cfg.data.sample_rate,
    ...
    train_dataset = ConcatDataset([train_dataset, train_dataset_additional])
    ...
    for epoch in range(first_epoch, num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            ...
            with accelerator.accumulate(net):
                ...
                flows = batch["flow_video"]
                bs = flows.shape[0]
                flows = rearrange(flows, "b f c h w -> (b f) c h w")
                flows = flow_to_image(flows) / 127.5 - 1
                flows = rearrange(flows, "(b f) c h w -> b c f h w", b=bs).to(weight_dtype)
                with torch.no_grad():
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % cfg.checkpointing_steps == 0 or global_step == 20:
                    if accelerator.is_main_process:
                        ...
                        accelerator.save(flow_guider.state_dict(),  os.path.join(save_path, f"flow_guider.pth"))
                        save_checkpoint(
                        ...
                        for tracker in accelerator.trackers:
                            if tracker.name == "tensorboard":
                                np_videos = torch.cat([d['vid'] for d in sample_dicts], dim=0) # (N, F, C, H, W) 
                                tracker.writer.add_video(f"validation", np_videos, global_step, fps=8)
